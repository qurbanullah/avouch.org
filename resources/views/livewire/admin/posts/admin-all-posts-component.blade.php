<div class="sm:px-6 lg:px-8 pb-4 bg-gray-50 dark:bg-gray-800">
    <div style="min-height: 770px" class="sm:relative md:h-full flex-col justify-between">
        <div class="py-2">
            <div>
                <h1 class="text-2xl font-semibold leading-tight">Groups</h1>
            </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:bg-gray-700 dark:text-white">
                                <thead>
                                    <tr class="dark:bg-gray-800">
                                        ...
                                        <th class="px-6 py-3 text-left text-xs leading-4 font-medium uppercase tracking-wider">Introduction</th>
                                        ...
                                        <th class="px-6 py-3 text-left text-xs leading-4 font-medium uppercase tracking-wider">Banner Image</th>
                                        <th class="px-6 py-3 text-left text-xs leading-4 font-medium uppercase tracking-wider">User Id</th>
                                        <th class="px-6 py-3 text-left text-xs leading-4 font-medium uppercase tracking-wider">Group Id</th>
                                        <th class="px-6 py-3 text-left text-xs leading-4 font-medium uppercase tracking-wider">Package Id</th>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="px-6 py-4 text-sm whitespace-no-wrap">
                                                        ...
                                                        class="text-blue-500 hover:text-blue-400 dark:text-blue-300 dark:hover:text-blue-400"
                                                            target="_blank"
                                                            href="{{ route('posts.post-detail-component', $item->slug) }}">
                                                            {{ $item->slug }}
                                                        </a>
                                                    </td>
                                                    <td class="px-6 py-4 text-sm whitespace-no-wrap">
                                                        {!! Str::limit($item->introduction, $limit = 30, $end = '') !!}
                                                    </td>
                                                    <td class="px-6 py-4 text-sm whitespace-no-wrap">
                                                    ...
                                                    <td class="px-10 py-4 text-right text-sm">
                                                        <a
                                                            href="{{ route('posts.post-edit-component', $item->slug) }}"
                                                            target="_blank"
                                                            >
    ...
    <div>
        <x-jet-dialog-modal wire:model="modalFormVisible">
            ...
            <x-slot name="content">
                <div class="mt-4">
                    <x-jet-label for="title" value="{{ __('Title') }}" class="text-lg dark:text-white" />
                ...
                <div class="mt-4">
                    <x-jet-label for="slug" value="{{ __('Slug') }}" class="text-lg dark:text-white" />
                ...
                <div class="pt-4">
                    <x-jet-label for="groupId" value="{{ __('Groups') }}" />
                    <select id="groupId" wire:model="groupId" class="form-select block w-full mt-1 border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm dark:bg-gray-500">
                        <option value="Select Category">Select Group</option>
                        @if (!empty($groups))
                            @foreach($groups as $item)
                                <option class="text-sm capitalize" value={{ $item->id }}>{{ $item->slug }}</option>
                            @endforeach
                        @endif
                    </select>
                ...
                <div class="pt-4">
                    <label class="block text-base ">
                        <span class="text-gray-700 dark:text-gray-50">Select Profession Sub Category</span>
                        <select wire:model="packageId" class="w-96 form-select block mt-1 border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm dark:bg-gray-500">
                            <option value="Select Skill">Select Package</option>
                            @foreach($packages as $item)
                                <option class="text-sm" value={{ $item->id }}>{{ $item->package }}</option>
                            @endforeach
                        </select>
                    </label>
                </div>
    ...
    <div>
        <x-jet-dialog-modal wire:model="modalConfirmDeleteVisible">
            ...
            <x-slot name="content">
                {{ __('Are you sure you want to delete this group? Once the group is deleted, all of its resources and data will be permanently deleted.') }}
            </x-slot>
