<div class="sm:px-6 lg:px-8 bg-gray-50 dark:bg-gray-800">
    <div style="min-height: 770px" class="sm:relative md:h-full flex-col justify-between">
        <div class="">
            <div class="py-2">
                <div>
                    <h2 class="text-xl font-semibold leading-tight">Groups</h2>
                </div>
                                ...
                                <table class="min-w-full divide-y divide-slate-300 dark:divide-slate-600 dark:bg-gray-800 dark:text-white">
                                    <thead>
                                        <tr class="dark:bg-gray-700 ">
                                            ...
                                            <th class="px-6 py-3 text-left text-xs leading-4 font-bold uppercase tracking-wider">Short Description</th>
                                            <th class="px-6 py-3 text-left text-xs leading-4 font-bold uppercase tracking-wider">Long Description</th>
                                            <th class="px-6 py-3 text-left text-xs leading-4 font-bold uppercase tracking-wider">Banner Image</th>
                                            {{-- <th class="px-6 py-3 text-left text-xs leading-4 font-bold uppercase tracking-wider">images</th> --}}
                                            {{-- <th class="px-6 py-3 text-left text-xs leading-4 font-bold uppercase tracking-wider">Thumbnail</th> --}}
                                            ...
                                            @if ($searchedData->count())
                                                @foreach ($searchedData as $item)
                                                    <tr>
                                                        ...
                                                        <td class="px-6 py-2 text-sm whitespace-no-wrap">
                                                            {!! Str::limit($item->short_description, $limit = 30, $end = '') !!}
                                                        </td>
                                                        <td class="px-6 py-2 text-sm whitespace-no-wrap">
                                                            {!! Str::limit($item->long_description, $limit = 30, $end = '') !!}
                                                        </td>
                                                        <td class="px-6 py-2 text-sm whitespace-no-wrap">
                                                            {{ $item->banner_image }}
                                                        </td>
        ...
        <div>
            <x-jet-dialog-modal wire:model="modalFormVisible">
                ...
                <x-slot name="content">
                    <div class="mt-2">
                        <x-jet-label for="name" value="{{ __('Name') }}" class="text-sm" />
                        <x-jet-input id="name" class="block w-full" type="text" wire:model.debounce.800ms="name" />
                        @error('name') <span class="error text-red-500">{{ $message }}</span> @enderror
                    </div>
                    <div class="mt-2">
                        <x-jet-label for="category" value="{{ __('Slug') }}" class="text-sm" />
                        <div class="flex rounded-md shadow-sm">
                            <div class="flex rounded-md shadow-sm">
                                <span class="hidden md:inline-flex items-center px-3 rounded-l-md border border-r-0 border-gray-300 text-xs">
                                    {{ env('APP_URL') . '/groups/' }}
                                </span>
                                <x-jet-input  id="slug" wire:model="slug" class="form-input px-3 flex-1 block w-full rounded-l-md rounded-r-md md:rounded-l-none border border-r-0 border-gray-300 transition duration-150 ease-in-out sm:text-sm sm:leading-5 text-xs" placeholder="url-slug" />
                            </div>
                        </div>
                    ...
                    <div class="mt-2">
                        <label>
                            ...
                            <span class="ml-2 text-xs">Set as active</span>
                        </label>
                    </div>
                    <div class="mt-2">
                        <x-jet-label for="shortDescription" value="{{ __('Short Description') }}" class="text-sm dark:text-white" />
                        <x-jet-input id="shortDescription" class="block w-full" type="text" wire:model.debounce.800ms="shortDescription" />
                        @error('shortDescription') <span class="error text-red-500">{{ $message }}</span> @enderror
                    </div>
                    <div class="mt-2">
                        <x-jet-label for="longDescription" value="{{ __('Long Description') }}" class="text-sm" />
                        <div class="rounded-md shadow-sm">
                            <div class="">
                                <div class="body-content bg-gray-50 dark:bg-gray-700 text-sm" wire:ignore>
                                    <input id="x" type="hidden" name="content">
                                    <trix-editor
                                        input="x"
                                        name="longDescription"
                                        class="trix-content trix-past text-xs"
                                        style="height: 200px !important;  overflow-y: auto !important;"
                                        x-ref="x"
                                        wire:model.debounce.800ms="longDescription"
                                        wire:key="trix-content-unique-key"
                                        trix-attachment-add="$attachment"
                                    ></trix-editor>
                                </div>
                            </div>
                        </div>
                        @error('longDescription') <span class="error text-red-500">{{ $message }}</span> @enderror
                    </div>
                    {{-- <div class="mt-4">
                        <x-jet-label for="images" value="{{ __('Images') }}" class="text-sm" />
                        <x-jet-input id="images" class="block w-full" type="text" wire:model.debounce.800ms="images" />
                        @error('images') <span class="error text-red-500">{{ $message }}</span> @enderror
                    </div> --}}
                    {{-- <div class="mt-4">
                        <x-jet-label for="thumbnail" value="{{ __('Thumbnail') }}" class="text-sm" />
                        <x-jet-input id="thumbnail" class="block w-full" type="text" wire:model.debounce.800ms="thumbnail" />
                    ...
                    <div class="mt-2">
                        ...
                        >
                            ...
                            @if(!empty($this->bannerImage))
                                <div>
                                    <input type="file" value="{{ $this->bannerImage }}" wire:model="imageUploaded">
                                </div>
                                <div class="pt-4">
                                    <img src="{{ env('APP_URL') . '/groups-images/' . $this->bannerImage }}">
                                </div>
